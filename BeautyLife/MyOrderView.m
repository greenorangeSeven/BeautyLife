...
{
    MyOrder *order = [orderData objectAtIndex:section];
    UIView *headerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 120)];//创建一个视图
    ...
    order_no_Label.text = [NSString stringWithFormat:@"订单编号:%@",order.order_sn];
    
    [headerView addSubview:order_no_Label];
    
    UILabel *time_Label = [[UILabel alloc] initWithFrame:CGRectMake(5, 25, 180, 20)];
    ...
    time_Label.text = [NSString stringWithFormat:@"下单时间:%@",[Tool TimestampToDateStr:order.addtime andFormatterStr:@"yyyy-MM-dd HH:mm:ss"]];
   ...
   [headerView addSubview:status_Label];
    ...
    UILabel *express_Label = [[UILabel alloc] initWithFrame:CGRectMake(5, 65, 180, 20)];
    ...
    UILabel *express_no_Label = [[UILabel alloc] initWithFrame:CGRectMake(5, 85, 180, 20)];
    
    express_no_Label.backgroundColor = [UIColor clearColor];
    
    express_no_Label.font = [UIFont boldSystemFontOfSize:12.0];
    
    express_no_Label.textColor = [UIColor redColor];
       ...
       [order.status isEqualToString:@"未付款"])
    {
        ...
        btn.frame = CGRectMake(220, 75, 90, 30);
